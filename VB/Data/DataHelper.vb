...
'INSTANT VB NOTE: The variable start was renamed since Visual Basic does not handle local variables named the same as class members well:
			Dim start_Conflict As DateTime = Start.AddHours(20).AddMinutes(40)
			Dim newStart As DateTime = start_Conflict
			Dim newEnd As DateTime = newStart.AddHours(1.5)
			Dim appt = New AppointmentEntity() With {
				.AppointmentType = CInt(Math.Truncate(AppointmentType.Pattern)),
...
'INSTANT VB NOTE: The variable start was renamed since Visual Basic does not handle local variables named the same as class members well:
		...
		End Function
		#End Region
		Public Shared Function GetAppointments() As ObservableCollection(Of AppointmentEntity)
			Dim collection As New ObservableCollection(Of AppointmentEntity)()
			DataHelper.Gym().ToList().ForEach(Sub(x) collection.Add(x))
			...
			Return collection
		End Function
		Public Shared Function GetResources() As ObservableCollection(Of ResourceEntity)
			Dim collection As New ObservableCollection(Of ResourceEntity)()
